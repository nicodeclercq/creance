...
import { CreditDistribution, Credit } from "../../services/CalculationService";
        ...
        credits.map((d) => (
          <>
            ...
            {fold(
              ...
              (usr: Registered<User>) => (
                <Avatar color={usr.color} name={usr.name} image={usr.avatar} />
              )
            )(get(d.user))}
          </>
        ))
    ...
    <DefaultLayout title="page.summary.title">
      <Card>
        ...
        >
          {fold(
            () => (<></>) as ReactNode,
            (dist: Omit<CreditDistribution, "notDistributed">[]) =>
              users.map((user, i) => (
                <>
                  <div
                    style={{
