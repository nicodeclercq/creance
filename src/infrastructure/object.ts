...
type DeepMergeTwoTypes<T, U> = [T, U] extends [any[], any[]]
  ? Zip_DeepMergeTwoTypes<T, U>
  : [T, U] extends [{ [key: string]: unknown }, { [key: string]: unknown }]
  ? MergeTwoObjects<T, U>
  : T | U;

export const deepMerge = <O1 extends {}, O2 extends {}>(
  obj1: O1,
  obj2: O2
): DeepMergeTwoTypes<O1, O2> => {
  const map = new Map();
  ...
  return pipe(
    ...
    Record.mapWithIndex((key, value) =>
      Record.has(key, obj2) ? mergeValue(obj1[key], obj2[key]) : value
    ),
...
export const get =
  <A extends keyof B, B extends {}>(key: A) =>
  (obj: B) =>
    obj[key];
export const getOptional =
  <A extends string, B extends {} | undefined, C>(key: A, orElse?: () => C) =>
  (obj: B) => {
    const o = obj || {};
    return Record.has(key, o)
      ? o[key]
      : isDefined(orElse)
      ? orElse()
      : undefined;
...
export const omit = <R extends { [key: string]: unknown }, Key extends keyof R>(
  record: R,
  omitedKeys: readonly Key[]
) =>
  Record.filterMapWithIndex((key) =>
    includes(key)(omitedKeys) ? Option.none : Option.some(true)
  )(record) as Omit<R, Key>;
...
export const hasKey = <K extends string, T extends {}>(
  key: K,
  obj: T
