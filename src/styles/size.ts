...
type Property =
  ...
  | "min-height"
  | "max-height";
export const formatResponsiveSize = (
  property: Property,
  responsize: ResponsiveSize
) => getStyle(responsize, (v: ResponsiveSize) => `${property}: ${v};`);

type _Padding = keyof typeof VAR.SIZE.PADDING.HORIZONTAL;
...
export const getPaddingStyle = (padding: Padding | undefined) => {
  ...
  if (typeof padding === "string") {
    return `padding: ${VAR.SIZE.PADDING.VERTICAL[padding]}  ${VAR.SIZE.PADDING.HORIZONTAL[padding]};`;
    ...
    return `padding: ${y != null ? VAR.SIZE.PADDING.VERTICAL[y] : "0"} ${
      x != null ? VAR.SIZE.PADDING.HORIZONTAL[x] : "0"
    }`;
  }
};
